...
def render(df1, df2, arm, wide, km_model, df_clustered, km_scaler,
    ...
    if "silhouette_score" in df_plot.columns:
        ...
        st.plotly_chart(fig_sil_box, use_container_width=True)
    ...
    with col_radar:
        ...
        def safe_mean_mapped(col, mapping):
            if col in df_seg.columns and df_seg[col].dtype == object:
                return df_seg[col].map(mapping).dropna().mean()
            elif col in df_seg.columns:
                return df_seg[col].dropna().mean()
            return 0.5
    ...
    for i in range(best_k):
        ...
        def safe_val(col, divisor=1.0, mapping=None):
            if col not in df_c.columns:
                return 0.5
            if mapping and df_c[col].dtype == object:
                return df_c[col].map(mapping).dropna().mean() or 0.5
            return min(1, df_c[col].dropna().mean() / divisor)
